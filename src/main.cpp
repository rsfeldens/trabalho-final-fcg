...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00578803 - Rodrigo Salvadori Feldens", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
void drawCat(glm::mat4 model)
{
    ...
    model = Matrix_Translate(player_position.x, player_position.y, player_position.z) * Matrix_Rotate_Y(g_CatAngleY);
...
void CursorPosCallback(GLFWwindow *window, double xpos, double ypos)
{
    ...
    if (g_LeftMouseButtonPressed)
    {
        if (isFreeCamera)
        {
            ...
            {
                // skipamos para evitar tomar a camera
            }
            else
            {
